...
* 仅支持Android, 适配`美团买菜 5.34.1`, 需要在`AutoJS.fakeIdlefish 4.1.22.0322.alpha`下运行, 下载地址: [https://github.com/TonyJiangWJ/Auto.js]
...
# 更新记录
### 22/05/30
- [x] 连续提交[结算], 最短有效间隔 1155
- [x] 确认 59.993 点击是否还是未到开售时间, 结果, 还是提示未到开售时间,  调整为 -5ms开始点击
- [x] [免密支付]一段时期后, 会需要再次输入密码 , [请输入支付密码]
- [x] 尝试解决 不符合 购买条件 (可能是到了[确认订单]页面需要等待页面加载), 已增加500-1000ms延时
- [x] 单轮次数调整到300次
- [x] 优化购物车内无商品的判断逻辑
...
### 22/05/19
* 还没开放下单的情况下, 购物车界面里会直接弹出[我知道了]
* [提交订单]页面, 缺货的情况下, [继续支付]的场景下, [返回购物车] 也会出现, 而且还会先匹配到[返回购物车]
* [提交订单]页面, 貌似press(btn)会触发按钮的disable, btn.click()不会
